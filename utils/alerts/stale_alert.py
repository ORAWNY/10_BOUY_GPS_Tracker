...
class _Editor(QDialog):
    def __init__(self, spec: AlertSpec, host: Host, parent=None):
        ...
        self.interval_spin.setValue(int(p.get("interval_min", 5)))
        ...
        self.escalation_combo = QComboBox()
        self.escalation_combo.addItems(["No", "Yes"])
        self.escalation_combo.setCurrentIndex(1 if p.get("email_on_escalation", True) else 0)

        self.recovery_combo = QComboBox()
        self.recovery_combo.addItems(["No", "Yes"])
    ...
    def accept(self):
        ...
        p["threshold_min"] = amber  # legacy key
...
class StaleViewDialog(QDialog):
    ...
    def _build_gaps(self) -> pd.DataFrame:
        ...
        # now→last gap (when not scope_all)
    ...
    def _on_edit(self):
        try:
            ...
            if callable(cfg):
                ...
                from utils.alerts import REGISTRY
    ...
    def _export_report(self):
        ...
        try:
            ...
            out["time_local"] = parse_series_to_local_naive(out["time_local"]).dt.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def _rebuild(self):
        ...
        d_chart = d.sort_values("t_end")
        self.ax.plot(d_chart["t_end"], d_chart["gap_min"], linewidth=2)
        ...
        # -------- TABLE ORDER: NEWEST FIRST --------
        self.table.setRowCount(0)
        d_table = d.sort_values("t_end", ascending=False)
        for _, row in d_table.iterrows():
            r_i = self.table.rowCount()
            self.table.insertRow(r_i)
            self.table.setItem(r_i, 0, QTableWidgetItem(parse_series_to_local_naive(pd.Series([row["t_end"]]))[0].strftime("%Y-%m-%d %H:%M:%S")))
    ...
    def default_spec(self, host: Host) -> AlertSpec:
        return AlertSpec(
            ...
            payload={
                ...
                "also_tables": [],        # optional extra tables
            },
        )
    ...
    def evaluate(self, spec: AlertSpec, host: Host) -> EvalResult:
        ...
        if not observed_parts:
            return {
                "status": Status.OFF,
                "observed": 0.0,
                "summary": "no valid times in any selected table",
            }
        ...
        interval_min = int(p.get("interval_min", 5))
        ...
        summary = (
            f"{fmt_duration(combined_observed)} "
            f"(amber≥{fmt_duration(amber_s)}, red≥{fmt_duration(red_s)}) • "
