...
def create_ws_correlation_dashboard():
    # --- Load dataset ---
    ...
    def create_card(title, widget):
        ...
        return widgets.VBox([header, widget], layout=widgets.Layout(
            border='2px solid #007acc',  # colored frame
            padding='10px',
            margin='5px',
    ...
    def plot_ws_correlation(df, lidar_pairs):
        ...
        for i, (h1, h2) in enumerate(lidar_pairs):
            ...
            axes[i].plot(temp_df[col1], reg.predict(X),
                         color='red', linestyle='--', linewidth=1.5)

            slope, intercept, r2 = reg.coef_[0], reg.intercept_, reg.score(X, y)
            title_height = f"{h1}m" if h1 == h2 else f"{h1}m vs {h2}m"

            axes[i].set_title(f'HWS Correlation [{title_height}]', fontsize=11)
            axes[i].set_xlabel(f'ZX1899 @ {h1}m (m/s)', fontsize=10)
            axes[i].set_ylabel(f'ZX1970 @ {h2}m (m/s)', fontsize=10)
            axes[i].set_xlim(0, 35)
            axes[i].set_ylim(0, 35)
            axes[i].set_xticks(np.arange(0, 36, 5))
            axes[i].set_yticks(np.arange(0, 36, 5))
        ...
        fig.suptitle("Buoy-5 Wind Speed Correlation Dashboard",
                     fontsize=18, fontweight="bold", y=1.04, color="#007acc")
        fig.text(0.5, 0.99,
                 "ZX1899 vs ZX1970 - Wind Speed Correlation at Various Heights",
                 ha='center', fontsize=14, color='gray')
        plt.show()

    # --- Widgets ---
    start_ws = widgets.DatePicker(description="Start", value=df.index.min().date())
    end_ws   = widgets.DatePicker(description="End", value=df.index.max().date())
    out = widgets.Output()

    def update_dashboard(change=None):
        with out:
            clear_output(wait=True)
            df_ws = df.loc[str(start_ws.value):str(end_ws.value)]
            plot_ws_correlation(df_ws, lidar_pairs)

    for w in [start_ws, end_ws]:
    ...
    controls_card = create_card("Controls", widgets.HBox([start_ws, end_ws]))
...
# --- Public entry point ---
def build():
    return create_ws_correlation_dashboard()

# remove it later
#if __name__ == "__main__":
#    dashboard_widget = create_ws_correlation_dashboard()
